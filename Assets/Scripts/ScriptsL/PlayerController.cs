...
public class PlayerController : MonoBehaviour
{
    ...
    public bool isDashing = false;      //traje li dash
    [SerializeField]
    ...
    void Update()
    {
        if (isClimbable && Mathf.Abs(moveInput.y) > 0)
        {     //ako je igrač na ljestvi/lijani i kreće se vertikalno, onda se penje
            isClimbing = true;
        }
    }
            ...
            if (CanMove && !animator.GetBool(AnimationStrings.isDashing))
            {
                ...
                else
                {
                    ...
                    if (touchingDirections.IsGrounded                                         //može skočiti kad je uzemljen
                     ...
                     || (!touchingDirections.IsGrounded && coyotetimer > 0 && !IsWallSliding))
                    {                 //ili ako nije prosao coyoteTime otkad je pao s platforme
                ...
                {                              //kad se spusti tipka za skakanje dok se igrač još uzdiže
